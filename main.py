...
# ส่วนค้นหาและพิมพ์ PDF ย้อนหลัง (แก้ไขให้โชว์ 4 ฟิลด์ตามคำขอ)
with st.expander("🔍 ค้นหาและจัดการประวัติเอกสาร"):
    if not inv_df.empty:
        # สร้าง List สำหรับ Dropdown (เลขใบกำกับ | ชื่อลูกค้า | วันที่ | สถานะ)
        options = [
            f"{r['invoice_no']} | {r['customer']} | วันที่: {r['date']} | สถานะ: {r['doc_status']}" 
            for _, r in inv_df.iterrows()
        ]
        selected = st.selectbox("เลือกรายการประวัติ (เลขที่ | ลูกค้า | วันที่ | สถานะ)", [""] + options[::-1])
